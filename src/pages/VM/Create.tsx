...
import {
    ...
    Grid,
    FormControl,
    Typography, TextField, InputAdornment, InputLabel, MenuItem, Select, ButtonGroup, Box, Chip
} from "@mui/material";
...
import {arrayMove} from "../../api/Tool";
...
export default function Create() {
    ...
    const deleteStorage = (del_index: number) => {
        setInputStorages((prevState) => prevState.filter((_, index) => (index !== del_index)))
    }
    ...
    const deleteNIC = (del_index: number) => {
        setInputNICs((prevState) => prevState.filter((_, index) => (index !== del_index)))
    }
                        ...
                        <Card sx={{minWidth: 275}}>
                            <CardContent>
                                <FormControl sx={{m: 1, width: '30ch'}}>
                                    <InputLabel id="host-label" required>Host</InputLabel>
                                    <Select
                                    ...
                                    >
                                        {
                                            hosts?.map((host: Hosts) => (
                                                ...
                                                >{host.host_name}({host.user})</MenuItem>
                                            ))
                                        }
                                    </Select>
                                </FormControl>
                            ...
                            <Card sx={{marginLeft: 2, marginRight: 10}}>
                                    ...
                                    <br/>
                                    {
                                        inputStorages?.map((storage: any, index) => (
                                            <Box key={"storage_" + index}>
                                                    ...
                                                    <Select
                                                        ...
                                                        }}>
                                                        {
                                                            storages?.map((storage: Storages) => (
                                                                ...
                                                                >{storage.name}({storage.comment})</MenuItem>
                                                            ))
                                                        }
                                                    </Select>
                                                </FormControl>
                                                {
                                                    (
                                                        storages.find(storage => storage.name === inputStorages[index].name)?.option.is_iso ||
                                                        storages.find(storage => storage.name === inputStorages[index].name)?.option.is_cloudimg
                                                    )
                                                    &&
                                                    // inputStorages[index]. ?? ""
                                                    <FormControl sx={{m: 1, width: '30ch'}}>
                                                        <InputLabel id="image-list-select-label">Image List</InputLabel>
                                                        <Select
                                                            ...
                                                            {
                                                                storageLists.find(storageList => storageList.name === (inputStorages[index].name ?? ""))?.path?.map((storageList, idx) => (
                                                                    <MenuItem key={storageList} value={storageList}
                                                                    >{storageList}</MenuItem>
                                                                ))
                                                            }
                                                        </Select>
                                                    </FormControl>
                                                ...
                                                <TextField
                                                    ...
                                                    InputProps={{
                                                        endAdornment: <InputAdornment
                                                            position="end">MB</InputAdornment>,
                                                    }}
                                                    onChange={event => {
                                                        setInputStorages((prevState) => {
                                                                return prevState.map((oldItem, oldIdx) => {
                                                ...
                                                {
                                                    index !== 0 &&
                                                    <Button size="small" variant="contained" color={"error"} sx={{m: 2}}
                                                            onClick={() => deleteStorage(index)}>削除</Button>
                                                }
                                                <ButtonGroup>
                                                    {
                                                        index !== 0 &&
                                                        <Button size="small" onClick={() => {
                                                            let newArr = inputStorages.concat()
                                                    ...
                                                    {
                                                        (inputStorages.length !== 1 && inputStorages.length !== index + 1) &&
                                                        <Button size="small" onClick={() => {
                                                            let newArr = inputStorages.concat()
                                                            newArr[index + 1] = inputStorages[index]
                                                            newArr[index] = inputStorages[index + 1]
                                ...
                                <CardActions>
                                    <Button size="small" onClick={() => addStorage()}>追加</Button>
                                    <Button size="small" onClick={() => console.log(inputStorages)}>Show</Button>
                                </CardActions>
                            </Card>
                            <br/>
                            <Card sx={{marginLeft: 2, marginRight: 10, marginBottom: 1}}>
                                        ...
                                        inputNICs?.map((storage: any, index) => (
                                            <Box key={"nic_" + index}>
                                                ...
                                                <ButtonGroup>
                                                    ...
                                                    {
                                                        (inputNICs.length !== 1 && inputNICs.length !== index + 1) &&
                                                        <Button size="small" onClick={() => {
                                                            let newArr = inputNICs.concat()
                                                            newArr[index + 1] = inputNICs[index]
                                                            newArr[index] = inputNICs[index + 1]
                                ...
                                <CardActions>
                                    <Button size="small" onClick={() => addNIC()}>追加</Button>
                                </CardActions>
                            </Card>
                        </Card>
                        <br/>
