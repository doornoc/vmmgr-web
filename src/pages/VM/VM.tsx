...
export default function VM() {
    ...
    if (obj.type <= 2) {
      for (let i = 0; i < obj.vm_detail.length; i++) {
        if (
          tmpVMs?.find(
            (vms) =>
              vms.vm?.UUID === obj.vm_detail[i].vm.UUID &&
              vms.node === obj.vm_detail[i].node
          ) == null
        ) {
  ...
  const clickNoVNC = (ip: string, port: string) => {
    window.location.replace(
      restfulApiConfig.noVNCURL + '?host=' + ip + '&port=' + port + '&path='
    )
  }
  ...
  return (
    <Base>
      <Container component="main" sx={{mt: 10}}>
        ...
        <Button onClick={clickCreatePage}>Create</Button>
        <br/>
        <br/>
        <Grid container spacing={2}>
          {vms?.map((vm: VMDetail, index) => (
            <Grid key={"vm_detail_" + index} item xs={12} md={6}>
              <Card>
                <CardContent>
                  <Typography
                    sx={{fontSize: 14}}
